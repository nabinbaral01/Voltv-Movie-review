...
// POST { tmdb_id, kind: "watched" | "want_to_watch" }
// Ensures the movie exists locally, then upserts a WatchHistory row.
export async function POST(req: NextRequest) {
  ...
  if (!movie) {
    try {
      ...
      return NextResponse.json({ error: "Could not fetch movie from TMDB" }, { status: 502 });
    }
  }
...
// GET ?tmdb_id=...  → current user's status for that movie
export async function GET(req: NextRequest) {
  ...
  const movie = await prisma.movie.findUnique({
    where: { tmdb_id: tmdbId }, select: { id: true },
  });
