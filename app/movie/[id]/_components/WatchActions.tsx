...
export default function WatchActions({ tmdbId }: { tmdbId: number }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/watchlist/quick?tmdb_id=${tmdbId}`, { signal: ctrl.signal })
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        className={`h-9 w-9 rounded-full flex items-center justify-center border transition-all          ${watched
      ...
      <button
        ...
        className={`h-9 w-9 rounded-full flex items-center justify-center border transition-all          ${wanted
