...
export default async function FollowersPage(
  ...
  const myFollowing = me
    ? await prisma.follow.findMany({
        where:  { follower_id: me.id, following_id: { in: followerIds } },
  ...
  const users = follows.map((f) => ({
    ...
    bio:            f.follower.bio,
    xp_points:      f.follower.xp_points,
    is_blue_tick:   f.follower.is_blue_tick,
    followersCount: f.follower._count.followers,
    amFollowing:    iFollowSet.has(f.follower.id),
  }));
      ...
      <div className="flex pt-14">
        ...
        <main className="flex-1 min-w-0 max-w-3xl mx-auto pb-24 lg:pb-8 px-4 sm:px-6">
          <div className="flex items-center gap-3 py-4 border-b border-[#1E1E2E] mb-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">Followers</h1>
